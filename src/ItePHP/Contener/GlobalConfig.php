...
class GlobalConfig
{
    ...
    {
        $arguments = $this->getArguments($dataXML, array(
            'name',
            'class',
        ));
        $config = array();
        ...
        foreach ($dataXML as $xmlKey => $xmlValue) {
            switch ($xmlKey) {
                case 'config':
                    ...
                    break;
                case 'method':
                    $serviceMethods[] = $this->appendMethod($xmlValue, $arguments);
                    break;
                default:
                    throw new InvalidConfigKeyException($xmlKey);
            }
        ...
        $this->setServiceData($arguments['name'], $arguments + array(
                'config' => $config,
    ...
    private function appendEvent($dataXML)
    {
        ...
        $this->setEventData($arguments['bind'], array(
     ...
     * @since 0.1.0
     */
    private function appendMethod($dataXML)
    {
        $arguments = $this->getArguments($dataXML, array('name'));
        $config = array();
        $serviceArguments = array();
